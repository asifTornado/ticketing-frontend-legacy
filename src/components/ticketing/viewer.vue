...
<div class="flex flex-col w-auto p-14 h-auto left-[500px] top-[100px] z-[9999999] border border-solid border-black  fixed bg-white" v-if="reopenCheck">
   ...
   <div style="z-index:1233333333333333333333" class="overflow-y-scroll w-2/4 h-auto bg-white fixed top-[70px] left-[380px]  rounded-md p-5 pt-10 " v-if="reassignTicketCheck">
      <div class="flex flex-col w-full h-full bg-white overflow-y-scroll" >
                
         <div class="flex flex-row w-full h-full mt-2 mb-2 bg-white overflow-y-scroll">
            <table>
              <thead class="mb-10">
...
<div style="z-index:1233333333333333333333" class=" w-2/4 overflow-y-scroll h-auto bg-white fixed top-[50px] left-[380px]  rounded-md p-5 pt-10 border border-solid border-black" v-if="assignTicketCheck">
   <div class="flex flex-col w-full h-full bg-white overflow-y-scroll" >
...
<div class="flex flex-col  w-full justify-end h-auto ">     
  ...
  <div class=" "> 
   <button  class="bg-red-500 text-white font-bold mr-2  mt-2 p-2  rounded-sm shadow-sm shadow-black" v-if="ticket.raisedBy.mailAddress == this.authStore.getUser.mailAddress && ticket.actions[ticket.actions.length - 1].type == 11 && ticket.assignedTo" @click="rateCheck = true">Rate</button>
   <button  class="bg-red-500 text-white font-bold mr-2  mt-2 p-2  rounded-sm shadow-sm shadow-black" v-if="((ticket.ticketingHead && this.authStore.getUser.empName == ticket.ticketingHead.empName ) || this.authStore.getUser.userType == 'admin') && ticket.assignedTo != null" @click="unassign($event, ticket)">Unassign</button>
   <button  class="bg-fuchsia-500 text-white font-bold mr-2  mt-2 p-2  rounded-sm shadow-sm shadow-black" @click="reassignTicketToggle"  v-if="((ticket.ticketingHead && this.authStore.getUser.empName == ticket.ticketingHead.empName)|| this.authStore.getUser.userType == 'admin') && ticket.assignedTo != null">Reassign</button>
   <button class="bg-blue-500 text-white font-bold mr-2  mt-2 p-2 rounded-sm" @click="assignTicketToggle" v-if="ticket.assignedTo == null && ((ticket.ticketingHead && this.authStore.getUser.mailAddress == ticket.ticketingHead.mailAddress) || this.authStore.getUser.userType == 'admin')">Assign </button>
   <button class="bg-emerald-500 text-white font-bold mr-2  mt-2 p-2 rounded-sm" @click="assignSelfToggle" v-if="ticket.assignedTo == null && (ticket.users.includes(this.authStore.getUser.mailAddress) || this.authStore.getUser.userType == 'admin')">Assign To Self</button>
    <button class="bg-amber-500 text-white font-bold mr-2  mt-2 p-2 rounded-sm" @click="showDetails">Show Details</button>
    <button class="bg-green-500 text-white font-bold mr-2  mt-2 p-2 rounded-sm" @click="closeTicket" v-if="ticket.status != 'Closed Ticket' && this.authStore.getUser.mailAddress == ticket.raisedBy.mailAddress">Close Ticket</button> 
    ...
    <button class="bg-purple-500 text-white font-bold mr-2  mt-2 p-2 rounded-sm" @click="reopenCheck = true" v-if="ticket.status == 'Closed Ticket' && this.authStore.getUser.mailAddress == ticket.raisedBy.mailAddress" >Open Again</button>       
...
this.$refs.commentBox.scrollTo({
  ...
  behavior: 'smooth'
      ...
      reopen(){
         ...
         axios.post(vm.globalUrl + "reOpen", data).then((result)=>{
